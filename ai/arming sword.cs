...
function AiPlayer::armingSwordAttack(%this) {
	...
	%distance = getSimTime() > %this.nextAttack ? 5 : 8;
	...
	if(getSimTime() > %this.nextAttack && vectorDist(%targetPosition, %position) < 7 && %this.target.isGrounded && %this.isGrounded && %this.canAttack()) {
		// attack
	...
	else if(getSimTime() > %this.nextDash && vectorDist((vectorAdd((getWords(%targetPosition, 0, 1)), (getWords(%target.getVelocity(), 0, 1)))), (getWords(%position, 0, 1))) < 5) {
		// intentionally dash to left/right
		%this.setMoveX(0);
		%this.setMoveY(0);
		%scalar =  - 25;
...
function AiPlayer::armingSwordDashToTarget(%this, %scalar, %useRightVector) {
	...
	%normalized = vectorNormalize(vectorSub(getWords(%this.target.getPosition(), 0, 1), getWords(%this.getPosition(), 0, 1)));
	if(%useRightVector) {
		%normalized = vectorNormalize(vectorCross(%normalized, "0 0 1"));
	}
	%velocity = vectorScale(%normalized, %scalar);
